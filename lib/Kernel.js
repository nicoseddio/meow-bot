...
class Kernel {
    ...
    distribute(message,mEvent = false) {
        let g = message.guild.id, c = message.channel.id;
        if (this.verifyStructure(this.config,'guilds',g,'apps')) {
            for (app in this.config.guilds[g].apps) {
                if (app.length === 0 || (app.length > 0 && app.includes(c)))
                    if (mEvent) app.handleMEvent(mEvent,message);

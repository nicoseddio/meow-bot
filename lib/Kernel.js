...
class Kernel {
    ...
    initialize(client) {
        ...
        console.log()
        console.log(this.appEnabled('CatMod.js','726982067136757853'))
    }
    distribute(message) {
    ...
    verifyStructure(parent, ...children) {
        let cs = children.join(';').split(';');
        console.log(parent + " " + cs)
        if (cs.length > 1 && cs[0] in parent) {
            let c = cs.shift()
            if (this.verifyStructure(parent[c],cs.join(';')))
                return true
            else return false
        }
    ...
    // NEEDS REWRITE to account for '.' in app names, refer to verifyStructure
